...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <div className="p-6">
      <h1 className="mb-4 text-lg font-semibold text-foreground">{t("Search")}</h1>
      <form className="mb-6 max-w-xl">
        <div className="flex h-9 items-center gap-2 rounded-md border border-border bg-surface-raised px-3">
          ...
          <input
            name="q"
            defaultValue={q}
            placeholder={t("Search projects, tasks, contacts, notes, decisions, email, meetings…")}
            autoFocus
      ...
      {!q ? (
        <p className="text-sm text-subtle">{t("Type at least 2 characters and press enter.")}</p>
      ) : results.length === 0 ? (
        <EmptyState title={t("No matches")} description={t('Nothing found for "{q}".', { q })} />
      ) : (
        <div className="max-w-2xl space-y-5">
          {Object.entries(grouped).map(([type, items]) => (
            <Panel key={type} title={`${t(TYPE_LABEL[type] ?? type)} (${items.length})`}>
              {items.map((r) => (
                <Link key={`${r.type}-${r.id}`} href={r.href} className="flex items-center gap-3 border-b border-border px-4 py-2.5 last:border-0 hover:bg-surface-raised">
                  <Badge tone="neutral">{t(TYPE_LABEL[r.type] ?? r.type)}</Badge>
