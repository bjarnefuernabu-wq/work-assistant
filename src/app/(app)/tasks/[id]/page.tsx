  ...
  return (
    <div className="p-6">
      <div className="mb-5 flex items-center justify-between">
        <h1 className="text-lg font-semibold text-foreground">{t("Edit task")}</h1>
          ...
          {task.dependsOn.map((d, i) => (
            <span key={d.id}>
              ...
              {d.dependsOnTask.status !== "COMPLETED" && (
                <span className="ml-1 text-critical">{t("(not done)")}</span>
              )}
            </span>
          ))}
        </div>
      ...
      <TaskForm action={boundAction} task={task} projects={projects} submitLabel={t("Save changes")} />
    </div>
  );
}
