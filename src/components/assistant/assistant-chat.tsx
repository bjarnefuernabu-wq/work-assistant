...
export function AssistantChat({ isLive }: { isLive: boolean }) {
  ...
  function confirm() {
    ...
    startTransition(async () => {
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: outcome.status === "SUCCESS" ? `Done — ${outcome.resultSummary}` : `That failed: ${outcome.resultSummary}`,
        },
      ]);
    });
  }
  ...
  function cancel() {
    ...
    startTransition(async () => {
      ...
      setMessages((prev) => [...prev, { role: "assistant", content: "Cancelled — I didn't make that change." }]);
    });
  }
    ...
    <div className="flex h-full flex-col">
      {!isLive && (
        <div className="border-b border-warning/30 bg-warning/5 px-4 py-2 text-xs text-warning">
          Running in offline fallback mode (no ANTHROPIC_API_KEY set) — answers only cover a few
          question types. Add a key in Settings for full assistant capability.
        </div>
      )}
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-4">
        {messages.length === 0 && (
          <div className="space-y-2">
            <p className="text-xs text-subtle">Try asking:</p>
          ...
          if (m.role === "assistant" && m.toolCallId) {
            return (
              <div key={i} className="flex items-center gap-1.5 text-[11px] text-subtle">
                <Wrench className="h-3 w-3" /> checking {m.toolName}…
              </div>
            );
          }
          return (
        ...
        {pending && (
          <div className="rounded-lg border border-warning/30 bg-warning/5 px-3.5 py-2.5">
            <p className="mb-2 text-xs text-foreground">
              <span className="font-semibold text-warning">Confirmation needed:</span> {pending.description}
            </p>
        ...
        {isSending && !pending && <p className="text-xs text-subtle">Thinking…</p>}
        <div ref={bottomRef} />
      </div>
      ...
      >
        <input
          ...
          placeholder="Ask about your projects, tasks, or week…"
